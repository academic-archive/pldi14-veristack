...
Function outcome_trace
          (out: outcome) (f: ident) {struct out}: trace :=
  match out with
    | Out_tailcall_return _ => E0
    | _ => Event_return f :: E0
  end.
...
Inductive eval_funcall:
  ...
  | eval_funcall_internal:
      ...
      forall tCR (HTCR: tCR = Event_call f.(fn_id) :: t ** outcome_trace out f.(fn_id)),
      eval_funcall m (Internal f) vargs tCR m3 vres
...
with exec_stmt:
  ...
  | exec_Stailcall:
      ...
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->      
      eval_funcall m' fd vargs t m'' vres ->
      forall tCR (HTCR: tCR = Event_return f.(fn_id) :: t),
      exec_stmt f (Vptr sp Int.zero) e m (Stailcall sig a bl) tCR e m'' (Out_tailcall_return vres).
...
CoInductive evalinf_funcall:
  ...
  | evalinf_funcall_internal:
      ...
      forall tCR (HTCR: tCR = (Event_call f.(fn_id) :: E0) *** t),
      evalinf_funcall m (Internal f) vargs tCR
...
with execinf_stmt:
  ...
  | execinf_Stailcall:
      ...
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->
      evalinf_funcall m' fd vargs t ->
      forall tCR (HTCR: tCR = (Event_return f.(fn_id) :: nil) *** t),
      execinf_stmt f (Vptr sp Int.zero) e m (Stailcall sig a bl) tCR.
...
(* funcall internal *)
  ...
  subst vres. apply star_one. subst S. eapply step_skip_call; eauto. 
  reflexivity. assumption.
  (* Out_return None *)
  eapply star_left. econstructor; eauto. 
  eapply star_trans. eexact A.
  subst vres. replace k with (call_cont k') by congruence.
  subst S. apply star_one. apply step_return_0; auto.
  reflexivity. assumption.
  (* Out_return Some *)
  eapply star_left. econstructor; eauto. 
  eapply star_trans. eexact A.
  ...
  subst S. apply star_one. eapply step_return_1; eauto.
  reflexivity. assumption.
  (* Out_tailcall_return *)
  eapply star_left. econstructor; eauto. 
  eapply star_trans. eexact A.
  subst vres S. red in H4. subst m3. rewrite H6. apply star_refl.
  reflexivity.
  assumption.
...
(* tailcall *)
  econstructor; split.
  eapply star_left. econstructor; eauto.  
  apply H5. apply is_call_cont_call_cont. assumption. 
  econstructor. 
Qed.
