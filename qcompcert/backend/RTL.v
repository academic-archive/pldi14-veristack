...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Itailcall:
      ...
      step (State s f (Vptr stk Int.zero) pc rs m)
        (Event_return FID :: E0) (Callstate s fd rs##args m')
  | exec_Ibuiltin:
  ...
  | exec_Ireturn:
      ...
      forall FID (HFID: FID = f.(fn_id)),
      step (State s f (Vptr stk Int.zero) pc rs m)
        (Event_return FID :: E0) (Returnstate s (regmap_optget or Vundef rs) m')
  | exec_function_internal:
      forall s f args m m' stk,
      Mem.alloc m 0 f.(fn_stacksize) = (m', stk) ->
      forall FID (HFID: FID = f.(fn_id)),
      step (Callstate s (Internal f) args m)
        (Event_call FID :: E0) (State s
                  f
                  (Vptr stk Int.zero)
                  f.(fn_entrypoint)
...
Proof.
  intros. constructor; simpl; intros. inversion H; subst; try (inv H0; now eauto).
