...
Lemma save_callee_save_correct:
  ...
  exists rs', exists m',
    star step tge 
       (State cs fb (sp) (save_callee_save fe k) rs m)
    E0 (State cs fb (sp) k rs' m')
  /\ (forall r,
...
Lemma function_prologue_correct:
  forall j ls ls0 rs m1 m1' m2 sp parent cs fb k,
  agree_regs j ls rs ->
  agree_callee_save ls ls0 ->
  wt_locset ls ->
  Mem.inject j m1 m1' ->
  Mem.alloc m1 0 f.(Linear.fn_stacksize) = (m2, sp) ->
  Val.has_type parent Tint -> True ->
  exists j', exists rs', exists sp', exists m4', exists m5',
     Mem.alloc m1' 0 tf.(fn_stacksize) = (m4', sp')
  /\ star step tge 
         (State cs fb sp' (save_callee_save fe k) (undef_temps rs) m4')
...
Proof.
  ...
  assert (SEP: forall b0 delta, j' b0 = Some(sp', delta) -> b0 = sp /\ delta = fe_stack_data fe).
    ...
    contradiction.
  (* Conclusions *)
  exists j'; exists rs'; exists sp'; exists m2'; exists m5'.
  split. auto.
  (* saving of registers *)
  split. eexact STEPS.
  (* agree_regs *)
...
Lemma function_epilogue_correct:
  forall j ls ls0 m sp m' sp' pa cs fb rs k m1,
  agree_regs j ls rs ->
...
Proof.
  ...
  inv MG. rewrite DOMAIN in H2. inv H2. unfold Int.eq. rewrite zeq_true. rewrite A. reflexivity.  eapply FUNCTIONS; eauto. 
...
Lemma transf_function_id: forall f tf, transf_function f = OK tf ->
                                       Linear.fn_id f = fn_id tf.
Proof.
  intros. exploit unfold_transf_function; eauto. intros; subst; reflexivity.
Qed.
...
(*  rewrite (unfold_transf_function _ _ TRANSL). unfold fn_link_ofs. 
  ...
  set (idx := match sl with
              ...
              | Incoming ofs ty => (*dummy*) FI_arg ofs ty
...
(*  exploit return_address_offset_exists. eexact IST.
  ...
  intros [rs1 [m1' [R [S [T [U [V W]]]]]]].
  econstructor; split.
  eapply plus_right. eexact S. econstructor; try eapply transf_function_id; eauto.
  ...
  intros [j' [rs' [sp' [m2' [m5' [A [D [E [F [G [J [K L]]]]]]]]]]]].
  econstructor; split.
  eapply plus_left. econstructor; try eapply transf_function_id; eauto.
