...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Ltailcall:
      ...
      step (State s f (Vptr stk Int.zero) pc rs m)
        (Event_return FID :: E0) (Callstate s f' (List.map rs args) m')
  | exec_Lbuiltin:
  ...
  | exec_Lreturn:
      ...
      forall FID (HFID: FID = f.(fn_id)),
      step (State s f (Vptr stk Int.zero) pc rs m)
        (Event_return FID :: E0) (Returnstate s (locmap_optget or Vundef rs) m')
  | exec_function_internal:
      forall s f args m m' stk,
      Mem.alloc m 0 f.(fn_stacksize) = (m', stk) ->
      forall FID (HFID: FID = f.(fn_id)),
      step (Callstate s (Internal f) args m)
        (Event_call FID :: E0) (State s f (Vptr stk Int.zero) f.(fn_entrypoint) (init_locs args f.(fn_params)) m')
