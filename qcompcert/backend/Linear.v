...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Ltailcall:
      ...
      step (State s f (Vptr stk Int.zero) (Ltailcall sig ros :: b) rs m)
        (Event_return FID :: E0) (Callstate s f' (return_regs (parent_locset s) rs) m')
  | exec_Lbuiltin:
  ...
  | exec_Lreturn:
      ...
      step (State s f (Vptr stk Int.zero) (Lreturn :: b) rs m)
        (Event_return FID :: E0) (Returnstate s (return_regs (parent_locset s) rs) m')
  | exec_function_internal:
      forall s f rs m m' stk,
      Mem.alloc m 0 f.(fn_stacksize) = (m', stk) ->
      forall FID (HFID: FID = f.(fn_id)),
      step (Callstate s (Internal f) rs m)
        (Event_call FID :: E0) (State s f (Vptr stk Int.zero) f.(fn_code) (call_regs rs) m')
