...
Definition not_stuck (L: semantics) :=
  forall beh, program_behaves L beh ->
              not_wrong beh.

Lemma not_stuck_backward: forall L1 L2, backward_simulation L1 L2 -> not_stuck L1 -> not_stuck L2.
Proof.
...
Lemma behaviors_single_events_prefix_inv:
  forall sem (SINGL: single_events sem) t beh (PREF: behavior_prefix t beh)
         S (BEHS: state_behaves sem S beh),
  exists S' beh',
    Star sem S t S' /\ state_behaves sem S' beh' /\ beh = behavior_app t beh'.
Proof.
  destruct beh; destruct 1 as [beh' Hbeh']; destruct beh' as [ | | T |]; try discriminate; inv Hbeh'.
  (* terminates *)
  inversion 1; clear BEHS; subst.
  exploit star_single_events_app_inv. eassumption. eassumption. reflexivity.
  destruct 1 as [? [? ?]].
  esplit. esplit.
  split. eassumption.
  split. eapply state_terminates. eassumption. eassumption.
  reflexivity.
  (* diverges *)
  inversion 1; clear BEHS; subst.
  exploit star_single_events_app_inv. eassumption. eassumption. reflexivity.
  ...
  esplit. esplit.
  split. eassumption.
  split. eapply state_diverges. eassumption. eassumption.
  reflexivity.
  (* reacts *)
  intros.
  assert (FOREVER: Forever_reactive sem S (t *** T)) by (inv BEHS; assumption).
  clear BEHS.
  revert T S FOREVER.
  induction t; simpl.
   intros; esplit; eexists (Reacts _); eauto using star_refl, state_reacts.
  intros.
  assert (FOREVER': exists t_, exists S_, exists T_, Star sem S t_ S_ /\ t_ <> E0 /\ Forever_reactive sem S_ T_ /\ t_ *** T_ = Econsinf a (t *** T)) by (inv FOREVER; eauto 7).
  clear FOREVER.
  destruct FOREVER' as [t_ [S_ [T_ [? [NE [? EQ]]]]]].
  destruct t_.
   destruct NE; reflexivity.
  injection EQ; clear EQ NE. intros EQ ?. subst.
  ...
  exploit (IHt T).
   ...
   assumption.
  intros [? [beh' [? [? ?]]]].
  esplit. esplit. split. eapply star_trans. eapply plus_star. eassumption. eassumption. reflexivity. split. eassumption. destruct beh'; try discriminate. simpl in *. congruence.
  (* goes wrong *)
  inversion 1; clear BEHS; subst.
  exploit star_single_events_app_inv. eassumption. eassumption. reflexivity.
  destruct 1 as [? [? ?]].
  esplit. esplit.
  split. eassumption.
  split. eapply state_goes_wrong. eassumption. eassumption. assumption.
  reflexivity.
Qed.
