...
Inductive event: Type :=
  ...
  | Event_call: ident -> event
  | Event_return: ident -> event
.

Function event_is_call_return (e: event) {struct e}: bool :=
  match e with
    | Event_call _ | Event_return _ => true
    | _ => false
  end.
...
Function trace_is_not_call_return (t: trace) {struct t}: bool :=
  match t with
    | nil => true
    | e :: t' => if event_is_call_return e then false else trace_is_not_call_return t'
  end.
...
Inductive match_traces: trace -> trace -> Prop :=
  ...
  | match_traces_call: forall id,
                         match_traces (Event_call id :: nil) (Event_call id :: nil)
  | match_traces_return: forall id,
                           match_traces (Event_return id :: nil) (Event_return id :: nil)
.
...
(** External call cannot modify memory unless they have [Max, Writable]
  ...
  (** External call cannot produce call/return events *)
  ec_not_call_return:
    forall F V (ge: Genv.t F V) vargs m1 t vres m2,
      sem F V ge vargs m1 t vres m2 ->
      trace_is_not_call_return t = true; 
