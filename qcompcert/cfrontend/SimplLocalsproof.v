...
Lemma transf_function_id: forall f tf, transf_function f = OK tf ->  fn_id f = fn_id tf.
Proof.
  unfold transf_function. intros. destruct (assertion
         (list_disjoint_dec ident_eq (var_names (fn_params f))
            (var_names (fn_temps f)))); try discriminate; simpl in *. destruct (simpl_stmt (cenv_for f) (fn_body f)); try discriminate; simpl in *. inv H; reflexivity.
Qed.
Local Hint Resolve transf_function_id.
