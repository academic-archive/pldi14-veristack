...
Lemma deref_loc_determ:
  forall ty m b ofs v, deref_loc ty m b ofs v ->
  forall v', deref_loc ty m b ofs v' -> v = v'.
Proof.
  induction 1; inversion 1; congruence.
Qed.
...
Lemma assign_loc_determ:
  forall ty m b ofs v m'1, assign_loc ty m b ofs v m'1 ->
  forall m'2, assign_loc ty m b ofs v m'2 -> m'1 = m'2.
Proof.
  induction 1; inversion 1; congruence.
Qed.
...
Lemma alloc_variables_determ:
  forall e m vl e'1 m'1, alloc_variables e m vl e'1 m'1 ->
  forall e'2 m'2, alloc_variables e m vl e'2 m'2 -> (e'1, m'1) = (e'2, m'2).
Proof.
induction 1; inversion 1.
congruence.
subst. eapply IHalloc_variables.
congruence.
Qed.
...
(** Determinism of Clight expressions. *)

Lemma eval_expr_lvalue_determ:
     (forall e v, eval_expr e v -> forall v', eval_expr e v' -> v = v')
  /\ (forall e b i, eval_lvalue e b i -> forall b' i', eval_lvalue e b' i' -> (b, i) = (b', i')).
Proof.
...
+ inversion 2. congruence.
  inversion H1.
+ inversion 3. exploit H0. eexact H5. congruence.
  inversion H2.
+ inversion 4. exploit H0. eexact H7. congruence.
  inversion H3.
+ inversion 6. exploit H0. eexact H10.
               exploit H2. eexact H11. congruence.
  inversion H5.
+ inversion 4. exploit H0. eexact H5. congruence.
  inversion H3.
+ inversion 4; subst; try (inversion H; fail).
  exploit H0; eauto.
  injection 1. intros; subst.
  eauto using deref_loc_determ.
+ inversion 2; congruence.
+ inversion 4; congruence.
+ inversion 3. exploit H0. eexact H6. congruence.
+ inversion 5. exploit H0. eexact H7. congruence.
  congruence.
+ inversion 4. congruence.
  exploit H0. eexact H8. congruence.
Qed.

Lemma eval_expr_determ:
  forall e v, eval_expr e v -> forall v', eval_expr e v' -> v = v'.
Proof (proj1 eval_expr_lvalue_determ).

Lemma eval_lvalue_determ:
  forall e b i, eval_lvalue e b i -> forall b' i', eval_lvalue e b' i' -> (b, i) = (b', i').
Proof (proj2 eval_expr_lvalue_determ).
...
Lemma eval_exprlist_determ:
  forall l tyl vl, eval_exprlist l tyl vl ->
  forall vl', eval_exprlist l tyl vl' -> vl = vl'.
Proof.
induction 1; inversion 1.
reflexivity.
erewrite IHeval_exprlist; eauto.
f_equal. exploit eval_expr_determ.
eexact H. eexact H7.
congruence.
Qed.
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | step_return_0: forall f k e le m m',
      ...
      step (State f (Sreturn None) k e le m)
        (Event_return FID :: nil) (Returnstate Vundef (call_cont k) m')
  | step_return_1: forall f a k e le m v v' m',
      ...
      step (State f (Sreturn (Some a)) k e le m)
       (Event_return FID :: nil) (Returnstate v' (call_cont k) m')
  | step_skip_call: forall f k e le m m',
      is_call_cont k ->
      Mem.free_list m (blocks_of_env e) = Some m' ->
      forall FID (HFID: FID = f.(fn_id)),
      step (State f Sskip k e le m)
       (Event_return FID :: nil) (Returnstate Vundef k m')
  ...
  | step_internal_function: forall f vargs k m e le m1,
      ...
      step (Callstate (Internal f) vargs k m)
        (Event_call FID :: E0) (State f f.(fn_body) k e le m1)
...
Section FUNCTIONENTRYDETERM.
Hypothesis FUNCTIONENTRYDETERM:
  forall f vargs m e1 le1 m'1, function_entry f vargs m e1 le1 m'1 ->
  forall e2 le2 m'2, function_entry f vargs m e2 le2 m'2 -> (e1, le1, m'1) = (e2, le2, m'2).

Lemma step_determ:
  forall (s : state) (t1 : trace)
                  (s1 : state) (t2 : trace)
                  (s2 : state),
                step s t1 s1 ->
                step s t2 s2 ->
...
Ltac t :=
  match goal with
  | [ H: (_ = _) \/ (_ = _) |- _ ] => (destruct H; discriminate) || clear H
  | [ |- _ /\ _ ] => split; [ assumption || constructor |]
  | [ |- (_ = _) -> _ ] => let H := fresh in intros H; inv H; subst
  | [ |- _ /\ _ -> _ ] => intros [? ?]; subst
  | [ H1: assign_loc ?ty ?m ?b ?ofs ?v _
    , H2: assign_loc ?ty ?m ?b ?ofs ?v _
      |- _ ] => exploit assign_loc_determ; [ eexact H1 | eexact H2 | clear H1 H2]
  | [ H1: eval_expr ?e ?le ?m ?a _
    , H2: eval_expr ?e ?le ?m ?a _
      |- _ ] => exploit eval_expr_determ; [ eexact H1 | eexact H2 | clear H1 H2]
  | [ H1: eval_exprlist ?e ?le ?m ?a ?l _
    , H2: eval_exprlist ?e ?le ?m ?a ?l _
      |- _ ] => exploit eval_exprlist_determ; [ eexact H1 | eexact H2 | clear H1 H2]
  | [ H1: eval_lvalue ?e ?le ?m ?a _ _
    , H2: eval_lvalue ?e ?le ?m ?a _ _
      |- _ ] => exploit eval_lvalue_determ; [ eexact H1 | eexact H2 | clear H1 H2 ]
  | [ H1: function_entry ?f ?va ?m _ _ _
    , H2: function_entry ?f ?va ?m _ _ _
      |- _ ] => exploit FUNCTIONENTRYDETERM; [ eexact H1 | eexact H2 | clear H1 H2]
  | [ H1: external_call ?ef ?ge ?va ?m _ _ _
    , H2: external_call ?ef ?ge ?va ?m _ _ _
      |- _ ] => exploit external_call_determ; [ eexact H2 | eexact H1 |];
                exploit external_call_determ; [ eexact H1 | eexact H2 |];
                clear H1 H2
  | [ H1: ?x = Tfunction _ _, H2: ?x = Tfunction _ _ |- _ ] =>
      rewrite H1 in H2; inv H2; clear H1 H2
  | [ H1: ?x = Some _, H2: ?x = Some _ |- _ ] =>
      rewrite H1 in H2; inv H2; clear H1 H2
  | [ H: ?x = ?x -> _ |- _ ] => generalize (H (eq_refl _)); clear H
  | [ |- _ ] => contradiction || congruence
  end.

induction 1; inversion 1; subst; repeat t.
Qed.

End FUNCTIONENTRYDETERM.
...
Lemma semantics_determinate:
  forall (p: program), determinate (semantics2 p).
Proof.
constructor.
+ apply step_determ.
...
Lemma semantics2_receptive:
  forall (p: program), receptive (semantics2 p).
Proof.
  intros. constructor; simpl; intros.
(* receptiveness *)
  assert (t1 = E0 -> exists s2, step2 (Genv.globalenv p) s t2 s2).
    intros. subst. inv H0. exists s1; auto.
  inversion H; subst; auto.
  (* builtin *)
  exploit external_call_receptive; eauto. intros [vres2 [m2 EC2]]. 
