...
Parameter is_strong_align : unit -> bool. (* whether to align stack by 8 or 16 *)

Definition strong_align : Z := if is_strong_align tt then 16 else 8.

Lemma strong_align_pos: strong_align > 0.
Proof.
  unfold strong_align. destruct (is_strong_align tt); omega.
Qed.

Definition make_env (b: bounds) :=
  let olink := 4 * b.(bound_outgoing) in  (* back link -- CANCELLED *)
  ...
  let sz := align (oretaddr + 4) strong_align - 4  in
...
Proof.
  ...
  generalize (align_le (align (fe.(fe_stack_data) + b.(bound_stack_data)) 4 + 4) _ strong_align_pos) .
...
Lemma align_strong_align: (8 | strong_align).
Proof.
  unfold strong_align. destruct (is_strong_align tt).
...
(*  /\ (4 | fe.(fe_ofs_retaddr)) *)
  /\ (8 | fe.(fe_stack_data))
  /\ (4 | fe.(fe_size))
  /\ (strong_align | fe.(fe_size) + 4) (* to cope with the return address *)
.  
Proof.
  intros.
  unfold fe, make_env, fe_size, (* fe_ofs_link, *) (* fe_ofs_retaddr, *)
  ...
  set (x2 := x1 (* + 4 *)).
  assert (4 | x2). unfold x2; auto. (* apply Zdivide_plus_r; auto. exists 1; auto. *)
  ...
  replace (align (x8 + 4) strong_align - 4 + 4) with (align (x8 + 4) strong_align) by ring.
  generalize (align_divides (x8 + 4) _ strong_align_pos).
  intro.
  assert (4 | 8) by (exists 2; reflexivity).
  generalize (align_strong_align).
  intro.
  assert (4 | strong_align) by eauto using Zdivide_trans.
  assert (4 | align (x8 + 4) strong_align) by eauto using Zdivide_trans.
  assert (4 | align (x8 + 4) strong_align - 4).
   eapply Zdivide_plus_r.
   assumption.
   exists (-1); reflexivity.
  tauto.
Qed.
...
Extract Constant is_strong_align => "fun _ -> false".
