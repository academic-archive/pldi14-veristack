...
let asttype_aux = function AST.Tint -> "AST.Tint" | AST.Tfloat -> "AST.Tfloat"

let asttype p t =
  fprintf p "%s" (asttype_aux t)

let astannotarg p = function
  | AST.AA_arg ty -> fprintf p "(AST.AA_arg %s)" (asttype_aux ty)
  | AST.AA_int i -> fprintf p "(AST.AA_int "; coqint p i; fprintf p ")"
  | AST.AA_float f -> fprintf p "(AST.AA_float "; coqfloat p f; fprintf p ")"
...
let external_function p = function
  ...
  | EF_annot(text, targs) -> 
      ...
      fprintf p "(EF_annot %ld%%positive %a)" (camlint_of_positive text) (print_list astannotarg) targs
  | EF_annot_val(text, targ) ->
      assertions := (text, [AST.AA_arg targ]) :: !assertions;
